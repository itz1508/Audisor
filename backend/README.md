...
Audisor has six local, read-only tools for Codex:
...
- `normalize` combines the Inspection Artifact's Dossier and Handoff with an
  evidence-referencing LLM Statement. It excludes the safe source snapshot
  and does not become an execution or approval authority.
...
The inspect-to-replay workflow uses JSON artifacts outside the inspected
...
uv run --directory backend audisor normalize inspection.json llm-statement.json --json > normalization.json
...
Start a new Codex session after registration. It exposes six native tools:
`audisor_scan`, `audisor_inspect`, `audisor_trace`, `audisor_normalize`,
`audisor_validate`, and `audisor_replay`.
